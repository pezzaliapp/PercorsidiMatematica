...
(function(){
  ...
  // Generators (subset sufficiente e stabili)
  ...
  var currentRoute=[], currentPathName='', currentGradeKey='primaria2', currentQuestion=null;
  ...
  function populatePaths(){
    ...
    if (!p.options.length){
      var def = routeMap['primaria2']; Object.keys(def.paths).forEach(function(name){
        var opt = document.createElement('option'); opt.value = name; opt.textContent = name; p.appendChild(opt);
      });
    }
  }
  ...
  function startGame(){
    ...
    document.getElementById('levelName').textContent = currentPathName;
    document.getElementById('gradeName').textContent = info.name;
  ...
  function nextQuestion(resetTitle){
    ...
    currentQuestion = gen();
    if (resetTitle) document.getElementById('levelName').textContent = currentPathName;
    ...
    currentQuestion.choices.forEach(function(c,idx){
      ...
      btn.addEventListener('click', function(){ selectChoice(idx, btn); }, { once:true }); document.getElementById('choices').appendChild(btn);
    });
  }
  ...
  function selectChoice(idx, el){
    ...
    if (correct){ stars+=1; done+=1; document.getElementById('stars').textContent=stars; document.getElementById('progress').textContent=done; }
  ...
  function endGame(){
    ...
    document.getElementById('finalStars').textContent = stars.toFixed(1);
    document.getElementById('feedback').textContent = (stars >= total*0.9) ? 'Ottimo lavoro!' : (stars >= total*0.6) ? 'Ben fatto! Continua ad allenarti.' : 'Buon inizio! Riprova.';
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    // LIM & update buttons
    on($('limBtn'),'click', function(){ document.body.classList.toggle('lim'); this.textContent = document.body.classList.contains('lim') ? 'LIM: ON' : 'Modalità LIM'; document.getElementById('statusBar').style.display = document.body.classList.contains('lim') ? 'block' : 'none'; });
    ...
    // Populate selects (deterministico)
    fillGradeSelector();
    populatePaths();
    on($('livello'),'change', populatePaths);
    on($('refreshPathsBtn'),'click', function(){ fillGradeSelector(); populatePaths(); });

    // Buttons
    on($('playBtn'),'click', startGame);
    on($('howBtn'),'click', function(){ document.getElementById('intro').classList.add('hidden'); document.getElementById('howto').classList.remove('hidden'); });
    on($('backIntro'),'click', function(){ document.getElementById('howto').classList.add('hidden'); document.getElementById('intro').classList.remove('hidden'); });
    on($('menuBtn'),'click', function(){ document.getElementById('game').classList.add('hidden'); document.getElementById('intro').classList.remove('hidden'); });
    on($('againBtn'),'click', startGame);
    on($('homeBtn'),'click', function(){ document.getElementById('summary').classList.add('hidden'); document.getElementById('intro').classList.remove('hidden'); });
    ...
    if ($('percorso').options.length) $('percorso').selectedIndex = 0;
  });
